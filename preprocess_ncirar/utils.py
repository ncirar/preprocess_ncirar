...
def _get_avgwordlen(x):
	count = _get_charcount(x)/_get_wordcount(x)
	return count

def _get_stopwordscount(x):
	stopcount = len([t for t in x.split() if t in stopwords])
	return stopcount

def _get_hashtagcount(x):
	hashtags = len([t for t in x.split() if t.startswith('#')])
	return hashtags

def _get_mentioncount(x):
	mentions = len([t for t in x.split() if t.startswith('@')])
	return mentions

def _get_digitcount(x):
	digits = len([t for t in x.split() if t.isdigit()])
	return digits
...
def _remove_rarewords(x, n=20):
	...
	freq = pd.Series(text).value_counts()
	raren = freq.tail(n)
	return ' '.join([t for t in x.split() if t not in raren])
